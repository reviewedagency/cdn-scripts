...
const processFunnelData = () => {
  ...
  $('.funnel-competitor1-average-rating').attr({
    'data-from': 0,
    'data-to': localStorage.getItem('funnelCompetitor1AverageRating')
  })
  processAverageRatingStars(
  ...
  $('.funnel-competitor1-review-count').attr({
    'data-from': 0,
  ...
  if (Array.isArray(userSubscriptions) && userSubscriptions.length > 0) {
    ...
    if (unusedSubscriptionIndex >= 0) {
      ...
      if (fId && businessName && userSubscriptions.length === 1) {
        ...
        $('.add-google-profile-button').trigger('click')
