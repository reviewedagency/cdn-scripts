...
const businessName = getUrlParam('business_name') || localStorage.getItem('businessName');
...
$('a.start-trial-button, div.start-trial-button').on('click', async (e) => {
  if (getUrlParam('f_id')) {
    ...
    await fetch('https://thereviewplanner-api-npfcm.ondigitalocean.app/supabase/website-button-trial-click', options);
...
if (fIdExist && email && location.pathname === '/') {
  ...
  fetch('https://thereviewplanner-api-npfcm.ondigitalocean.app/supabase/website-view-event', options);
}
...
const convertImageToBase64 = async (imageUrl) => {
  try {
    ...
    const response = await fetch('https://insta-app-xzm2x.ondigitalocean.app/image/convert/base64', options);
...
const fetchBusinessInfo = async (fId) => {
  ...
  try {
    const response = await fetch(`https://thereviewplanner-api-npfcm.ondigitalocean.app/supabase/google-data/${fId}`, {
      method: 'GET',
    });
  ...
  if (email) {
    $('a').each(function () {
      if ($(this).attr('href') && $(this).attr('href') !== '#') {
        $(this).attr(
          'href',
          `${$(this).attr('href').split('?')[0]}?f_id=${fId}&business_name=${businessName}&email=${email}&sender=${sender}`,
        );
      }
    });

    fetch(`https://thereviewplanner-api-npfcm.ondigitalocean.app/supabase/google-data/verify-existence?fId=${fId}&email=${email}`, {
      method: 'GET',
    })
      ...
      .then((result) => {
        if (result.data) {
          if (getUrlParam('fId') === localStorage.getItem('fId')) {
            ...
            const timeDifference = localStorage.getItem('lastInfoFetch') - new Date().getTime();
...
const searchGoogleInfo = async ({ businessName, businessAddress }) => {
  try {
    ...
    const response = await fetch('https://thereviewplanner-api-npfcm.ondigitalocean.app/rapid-api/google/search-profile', {
      method: 'POST',
      headers: myHeaders,
      body: JSON.stringify({
        businessName,
...
$('a[href].start-trial-button')
  ...
  .on('click', function (e) {
    ...
    const clickedPlan = planId ? `${$('.pricing-tab-link.active').attr('id')}-${planId}` : currentPlan;
...
document.addEventListener('DOMContentLoaded', async function () {
  ...
  window.$memberstackDom.getCurrentMember().then((member) => {
    ...
    if (customFields['trial-started']?.toLowerCase() === 'yes' && customFields['trial-profile-added']?.toLowerCase() === 'no') {
      $('.add-account.step-2').show();
    }
  ...
  if (location.pathname.includes('dashboard')) {
    ...
    Webflow.push(function () {
      ...
      $('#add-google-profile-form').on('submit', async (e) => {
        ...
        googleProfiles = await searchGoogleInfo({ businessName, businessAddress });
        const profileElement = $('#select-profile .results-box').first().removeClass('green');
        if (googleProfiles?.length > 0) {
          ...
          googleProfiles.map((profile, index) => {
            ...
            if (profile.featuredImage) {
              clonedElement.find('.business-featured-image').attr('src', profile.featuredImage);
            } else {
    ...
    // TASK 7:
    $('#select-profile .select-profile-add-account').on('click', async function () {
      $('.select-profile-add-account').addClass('disabled');
      const selectedIndex = $('#select-profile .results-box.green').data('index');
      ...
      if (Array.isArray(currentJsonData?.subscriptions)) {
        ...
        const targetSubscriptionIndex = updatedSubscriptions.findIndex((subscription) =>
          Object.keys(subscription).every((key) => ['subscription_id', 'item_price_id_short', 'upgrade_section_shown'].includes(key)),
        );
        ...
        if (targetSubscriptionIndex >= 0) {
          ...
          await fetch('https://n8n.thereviewdirectory.com/webhook/b3305d99-c3c0-4d0a-a952-8e21859acbff', {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
            },
    ...
    $('#targeting-save-button').on('click', async function (e) {
      e.preventDefault();
      const selectedSubscriptionId = $('.current-business-name').data('subscription-id');
      $(this).text('Saving...').attr('disabled', true);
      const selectedGender = $('.custom-dropdown.gender .custom-dropdown-option.selected').data('value') || '';
      const selectedLanguages = [];
      $('.custom-dropdown.languages .custom-dropdown-option.selected').each(function () {
        selectedLanguages.push($(this).data('value'));
      });
      const selectedLocations = [];
      $('.custom-dropdown.locations .custom-dropdown-option.selected').each(function () {
    ...
    const processDashboardInfo = async (subscriptionId) => {
      ...
      subscriptions.forEach(async (subscription) => {
        ...
        clonedElement.find('.rating-container .star:lt(' + rating + ')').addClass('filled');
        clonedElement.find('.rating-container .star:gt(' + (rating - 1) + ')').removeClass('filled');

        if (subscription.business_picture) {
          const imageBase64 = await convertImageToBase64(subscription.business_picture);
          clonedElement.find('.business-picture').attr('src', imageBase64);
        }

        clonedElement.find('.manage-subscription').on('click', function (e) {
          e.preventDefault();
          const subscriptionId = $(this).closest('.account-dropdown-card').data('subscription-id');
          if (subscriptionId) {
      ...
      if (subscriptionId) {
        currentSubscription = subscriptions.find((subscription) => subscription.subscription_id === subscriptionId);
      } else {
        currentSubscription = subscriptions[0];
      }

      if (currentSubscription) {
        $('.current-business-name').attr('data-subscription-id', currentSubscription.subscription_id);
        $('.current-business-name').text(currentSubscription.business_name);
        $('.current-business-address').text(currentSubscription.business_address);
        $('.current-business-rating').text(currentSubscription.business_rating);
        $('.business-review-count').text(currentSubscription.business_review_count);
        $('.current-plan').text(currentSubscription.item_price_id_short);

        const rating = Math.round(currentSubscription.business_rating);
        $('.business-rating-container .svg-default:lt(' + rating + ')').addClass('filled');
        $('.business-rating-container .svg-default:gt(' + (rating - 1) + ')').removeClass('filled');

        if (currentSubscription.business_picture) {
          const imageBase64 = await convertImageToBase64(currentSubscription.business_picture);
          $('.current-business-picture').attr('src', imageBase64);
        }

        $('.reviews-published').text(currentSubscription.reviews_published);
        $('.reviews-published-percentage').text(currentSubscription.reviews_published_percentage);
        $('.reviews-pending').text(currentSubscription.reviews_pending);
        $('.reviews-pending-percentage').text(currentSubscription.reviews_pending_percentage);
        $('.reviews-positive').text(currentSubscription.reviews_positive);
        $('.reviews-positive-percentage').text(currentSubscription.reviews_positive_percentage);
        $('.reviews-negative').text(currentSubscription.reviews_negative);
        $('.reviews-negative-percentage').text(currentSubscription.reviews_negative_percentage);
        ...
        const normalizeMonthlyReviews = () => {
          const monthlyReviews = Array.isArray(currentSubscription.monthly_reviews) ? currentSubscription.monthly_reviews : [];
        ...
        const renderReviewsChart = (range) => {
          ...
          const categories = filteredData.map((item) => formatMonthLabel(item.month));
          const negativeSeries = filteredData.map((item) => Number(item.negative) || 0);
          const positiveSeries = filteredData.map((item) => Number(item.positive) || 0);
          ...
          const chartOptions = {
            ...
            grid: { strokeDashArray: 4, borderColor: '#e0e0e0', xaxis: { lines: { show: true } } },
            xaxis: { categories },
            yaxis: { title: { text: 'Number of Reviews' }, min: 0, max: yAxisMax },
          ...
          if (window.reviewDashboardChart) {
            ...
            window.reviewDashboardChart = new ApexCharts(chartContainer, chartOptions);
        ...
        $('.dropdown-dash.analytics .dropdown_item-wrapper').on('click', function () {
          const selectedRange = $(this).data('value');
          const displayText = $(this).find('.dropdown_item_text').text();
          $('.dropdown-dash.analytics .dropdown_item_active_text').text(displayText);
        ...
        if (targetingGender) {
          $(`.custom-dropdown.gender .custom-dropdown-option[data-value="${targetingGender}"]`).click();
        }

        if (targetingLanguages.length > 0) {
          targetingLanguages.forEach((language) => {
            $(`.custom-dropdown.languages .custom-dropdown-option[data-value="${language}"]`).click();
          });
        }

        if (targetingLocations.length > 0) {
          targetingLocations.forEach((location) => {
            $(`.custom-dropdown.locations .custom-dropdown-option[data-value="${location}"]`).click();
          });
        }
